...
export default function ActivateAccount() {
  ...
  async function handleEmailSubmit(e) {
    ...
    try {
      // call auth service to request account activation
      const payload = { email: email.trim() }
      await authPost('activate-account', payload, { public: true })
      setStep('code')
    } catch (err) {
      setError(err.message || 'Failed to send activation email. Please try again.')
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleCodeSubmit(e) {
    ...
    try {
      ...
      const payload = { activation_code: entered, email: email.trim() }
      const resp = await authPost('verify-activation-code', payload, { public: true })
  ...
  return (
    <div className="forgot-page-wrapper">
      <div className="forgot-content">
        ...
        {step === 'email' ? (
          <>
            <h2 className="forgot-title">Activate your account</h2>
            <p className="forgot-subtitle">Enter your email to receive an activation code.</p>
          ...
          <>
            <h2 className="forgot-title">Enter your activation code</h2>
            ...
            <div className="forgot-back">
              If you didn't receive a code, <button type="button" className="link-button" onClick={async () => {
                ...
                setError('')
                try {
                  await authPost('activate-account', { email: email.trim() }, { public: true })
                } catch (err) {
          ...
          <>
            <div className="reset-container">
              <div className="reset-rules">
                <h2 className="forgot-title">Set Your Password</h2>
                ...
                <form onSubmit={(e) => e.preventDefault()}>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      setError('')
                      try {
                        await authPatch('set-password', { password: newPassword, confirm_password: confirmPassword, verification_token: opaqueToken }, { public: true })
                        navigate(ROUTES.LOGIN, { replace: true, state: { accountActivated: true } })
                      } catch (err) {
