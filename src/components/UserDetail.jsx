...
export default function UserDetail() {
  ...
  const fetchUser = async () => {
    ...
    setQuickGpaError('')
    try {
      const data = await get(`http://localhost:8083/api/v1/account-management/get-user/${id}`)
      setUser({
  ...
  const handleSave = async () => {
    ...
    try {
      if (isNewUser) {
        ...
        await post('http://localhost:8083/api/v1/account-management/create', payload)
        ...
        await put(`http://localhost:8083/api/v1/account-management/update-user/${user.uid}`, payload)
  ...
  const handleQuickGpaUpdate = async () => {
    ...
    try {
      await patch(`http://localhost:8083/api/v1/account-management/update-user-gpa/${user.uid}`, parsedGpa)
  ...
  const handleDelete = async () => {
    ...
    setError('')
    try {
      await deleteRequest(`http://localhost:8083/api/v1/account-management/delete/${user.uid}`)
      navigate('/')
    } catch (err) {
