					...
					<div className="lumos-nav" style={{ position: 'relative' }}>
						...
						{lumosOpen && (
							<div className="lumos-submenu" style={{ position: 'absolute', right: 0, top: 'calc(var(--nav-height) + 8px)', background: '#fff', borderRadius: 8, boxShadow: '0 10px 30px rgba(0,0,0,0.08)', padding: '8px', minWidth: 160, zIndex: 1400 }}>
								...
								<button className="lumos-submenu-btn" onClick={() => { setLumosOpen(false); navigate(ROUTES.LUMOS_AI, { state: { newChat: true } }) }} style={{ display: 'block', width: '100%', textAlign: 'left', padding: '8px 10px', border: 'none', background: 'transparent', cursor: 'pointer' }}>Start new chat</button>
							</div>
						)}
					</div>
